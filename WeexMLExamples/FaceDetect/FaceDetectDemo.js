	...
	module.exports = {
	  ...
	  methods: {
	    detect: function detect(e) {
	      ...
	      var faceDetect = weex.requireModule('faceDetect');
	      var self = this;
	      faceDetect.detectLandmarks(this.$refs.image.ref, function (results) {
	        _this.ctx.translate(0, 500);
	        _this.ctx.scale(1, -1);
	        results.forEach(function (result) {
	          ...
	          for (var key in result) {
	            if (result.hasOwnProperty(key) && key !== "boundingBox") {
	              ...
	              ctx.lineCap = "round";
	              var region = result[key];
	              var lastX = region[0].x;
	              var lastY = region[0].y;
	    ...
	    change: function change(e) {
	      ...
	      var index = Math.floor(Math.random() * 120);
